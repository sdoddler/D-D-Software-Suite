...
#include <GuiButton.au3>
 ...
 ;; TO DO
 ; Fix up Dice Roll. Allow non standard dice rolls and +3 (for saving throws etc.)
 ; Also add Queue - DO this with an Array (Title, Rolls, Sides, Addition) (Use global debug for all throws so people can disable half way through and its constant.)
 ;				  - You can then check what the Number in the queue is (for the button) via Ubound()
 ;				  - Create a clear button too, and allow users to roll non queue rolls as well.
 ;
...
Func FullSizeGui()
	...
	GUICtrlCreateLabel("Rolls:", 170, 330)
	$gRolls = GUICtrlCreateInput($diceAmount, 200, 328, 35, -1, $ES_NUMBER)

	GUICtrlCreateLabel("Sides:", 240, 330)
	$coDice = GUICtrlCreateCombo("", 270, 328, 40);;, -1, $CBS_DROPDOWNLIST)
	GUICtrlSetData(-1, "4|6|8|10|12|20|100", $diceType)

	GUICtrlCreateLabel("Add:", 320, 330)
	$gAdd = GUICtrlCreateInput(0, 345, 327, 39, 20, $ES_NUMBER)
	GUICtrlCreateUpdown(-1)
	GUICtrlSetLimit (-1, 10 , -10 )

	GUISetFont(14)

	$bRoll = GUICtrlCreateButton("Roll!", 425, 320)
	GUISetFont(8.5)
